...
import logging
from utils.ministral_llm import MinistralLLM
...
class AgentWorkflow:
    def __init__(self, model_name_or_path="mistralai/Ministral-8B-Instruct-2410", pipe=None, tokenizer=None):
        llm = MinistralLLM(model_name_or_path=model_name_or_path, pipe=pipe, tokenizer=tokenizer)
        self.researcher = ResearchAgent(llm=llm, pipe=pipe, tokenizer=tokenizer)
        self.verifier = VerificationAgent(llm=llm, pipe=pipe, tokenizer=tokenizer)
        self.relevance_checker = RelevanceChecker(llm=llm, pipe=pipe, tokenizer=tokenizer)
